...
import { OnramperLogo, MoonpayLogo } from './components/Logos';
import { StreetCoinzLogoBase64 } from './components/StreetCoinzLogo';
...
type Tab = 'home' | 'search' | 'wallet' | 'crypto-crash' | 'chicken-road' | 'admin' | 'dice' | 'roulette' | 'plinko' | 'mines';
        ...
        <>
          ...
          >
            ...
            <div className="space-y-2 flex-1">
              <SidebarItem icon={(props: any) => <Star {...props} fill="currentColor" />} label="Tier Status" onClick={() => { onClose(); onOpenTierStatus(); }} />
...
function Header({ onMenuClick, user, onSignInClick, activeTab, onOpenActivityNotification, unreadCount }: { onMenuClick: () => void, user: string | null, onSignInClick: () => void, activeTab: Tab, onOpenActivityNotification: () => void, unreadCount: number }) {
  return (
    <header className="fixed top-0 left-0 right-0 h-16 bg-[#131212]/90 backdrop-blur-md border-b border-white/5 flex items-center justify-between px-4 z-50">
      <a 
        href="https://www.streetcoinz.com" 
        className="block touch-manipulation"
        onContextMenu={(e) => {
          e.preventDefault();
          window.location.href = 'https://www.streetcoinz.com';
        }}
      >
        <img 
          src={StreetCoinzLogoBase64} 
...
const HomeScreen = ({ markets, onBet, onSelectMarket, onStartPlaying, onSelectOriginalGame, onOpenTerms, onOpenPrivacy, onOpenReferral, onOpenAuth, liveBets, isLoggedIn, userVolume = 1250 }: { markets: Market[], onBet: (market: Market, direction: 'UP' | 'DOWN') => void, onSelectMarket: (market: Market) => void, onStartPlaying: () => void, onSelectOriginalGame: (gameId: string) => void, onOpenTerms: () => void, onOpenPrivacy: () => void, onOpenReferral: () => void, onOpenAuth: () => void, liveBets: LiveBet[], isLoggedIn?: boolean, userVolume?: number }) => {
  ...
  const getTierInfo = (volume: number) => {
    if (volume < 5000) return { name: 'Rookie', next: 'Grinder', max: 5000, color: 'text-white', bg: 'bg-white', from: 'from-white', to: 'to-gray-300' };
    if (volume < 20000) return { name: 'Grinder', next: 'Tactician', max: 20000, color: 'text-[#8B4513]', bg: 'bg-[#8B4513]', from: 'from-[#8B4513]', to: 'to-[#A0522D]' };
        ...
        <div className="relative">
          <div 
            ...
            onScroll={(e) => {
              ...
              // We need to add state for this in the App component
          ...
          >
            {/* Slide 1: Tier Status / Crypto Games Arena */}
            <div className="min-w-full snap-center shrink-0">
              {isLoggedIn ? (
                <div className="bg-[#1e1e1e] rounded-xl p-5 border border-white/10 relative overflow-hidden shadow-lg h-full">
                    ...
                    <div className="flex justify-between items-end mb-3">
                      <div>
                        <div className="inline-flex items-center gap-1.5 bg-white/5 px-2 py-1 rounded-md mb-2 border border-white/5">
                          <Star className={`w-3 h-3 ${tierInfo.color}`} fill="currentColor" />
                          <span className="text-[10px] font-bold tracking-wider uppercase text-gray-300">Tier Status</span>
                        </div>
                        <h3 className={`text-xl font-bold mb-0.5 flex items-center gap-2 ${tierInfo.color}`}>
                ...
                <div className="bg-gradient-to-r from-[#a252f0] to-purple-600 rounded-xl p-4 text-white shadow-lg shadow-purple-900/20 relative overflow-hidden border border-white/10 h-full">
                  <div className="relative z-10">
            ...
            {/* Slide 2: Invite Friends */}
            <div className="min-w-full snap-center shrink-0">
              <div className="bg-gradient-to-r from-emerald-500 to-teal-600 rounded-xl p-4 text-white shadow-lg shadow-emerald-900/20 relative overflow-hidden border border-white/10 h-full">
                <div className="relative z-10">
                  <div className="inline-flex items-center gap-1 bg-black/30 backdrop-blur-md px-2 py-1 rounded-full mb-2 border border-white/10">
                    <Gift className="w-3 h-3 text-white" />
                    <span className="text-[10px] font-medium text-white">Referral Program</span>
                  </div>
                  <h3 className="text-lg font-bold mb-1">Invite Friends & Get 3000 USDT</h3>
                  <p className="text-xs opacity-90 mb-4 max-w-[200px]">Invite your friends to play and get a bonus up to 3000 USDT.</p>
                  
                  <div className="flex items-center gap-3">
                    <button 
                      onClick={() => isLoggedIn ? onOpenReferral() : onOpenAuth()}
                      className="bg-white text-black px-4 py-2 rounded-lg text-xs font-bold hover:bg-gray-100 transition-colors shadow-lg"
                    >
                      Invite Now
                    </button>
          ...
          {/* Dots */}
          <div className="absolute bottom-0 left-0 right-0 flex justify-center gap-1.5 pb-1 pointer-events-none">
            <div className="hero-dot h-1 rounded-full transition-all duration-300 w-4 bg-white" />
            <div className="hero-dot h-1 rounded-full transition-all duration-300 w-1.5 bg-white/30" />
          </div>
        </div>
      </section>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-bold">USDT (BNB)</div>
                  <div className="text-xs text-gray-400">Tether on BNB Smart Chain</div>
                </div>
              </div>
              <div className="text-right">
                <div className="font-mono font-bold">{blockchainAssets.bnbUsdt.toFixed(4)}</div>
                <div className="text-xs text-gray-400 font-mono">{formatCurrency(blockchainAssets.bnbUsdt)}</div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5">
              <div className="flex items-center gap-3">
                <UsdtIcon className="w-8 h-8" />
                <div>
                  <div className="font-bold">USDT (ETH)</div>
                  <div className="text-xs text-gray-400">Tether on Ethereum</div>
                </div>
              </div>
              <div className="text-right">
                <div className="font-mono font-bold">{blockchainAssets.ethUsdt.toFixed(4)}</div>
                <div className="text-xs text-gray-400 font-mono">{formatCurrency(blockchainAssets.ethUsdt)}</div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5">
              <div className="flex items-center gap-3">
                <UsdtIcon className="w-8 h-8" />
                <div>
                  <div className="font-bold">USDT (Polygon)</div>
                  <div className="text-xs text-gray-400">Tether on Polygon</div>
                </div>
              </div>
              <div className="text-right">
                <div className="font-mono font-bold">{blockchainAssets.maticUsdt.toFixed(4)}</div>
                <div className="text-xs text-gray-400 font-mono">{formatCurrency(blockchainAssets.maticUsdt)}</div>
              </div>
            </div>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        {selectedBank === 'Onramper' ? (
                          <OnramperLogo className="h-6 w-auto text-white" />
                        ) : (
                          <MoonpayLogo className="h-6 w-auto text-white" />
                        )}
                      </div>
                      ...
                      <div className="absolute top-full left-0 right-0 mt-2 bg-[#1a1a1a] border border-white/10 rounded-xl overflow-y-auto z-50 shadow-xl max-h-48 custom-scrollbar">
                        {[
                          ...
                          >
                            <div className="flex items-center gap-3">
                              {bank.name === 'Onramper' ? (
                                <OnramperLogo className="h-6 w-auto text-current" />
                              ) : (
                                <MoonpayLogo className="h-6 w-auto text-current" />
                              )}
                            </div>
...
function TierStatusModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const tiers = [
    { name: 'Rookie', color: 'text-white', bg: 'bg-white/10', border: 'border-white/20', desc: 'Total Volume $0 - $5.000', icon: Star, benefits: [] },
    { name: 'Grinder', color: 'text-[#8B4513]', bg: 'bg-[#8B4513]/10', border: 'border-[#8B4513]/20', desc: 'Total Volume $5.000 - $20.000', icon: Star, benefits: ['Monthly Cashback'] },
    { name: 'Tactician', color: 'text-blue-500', bg: 'bg-blue-500/10', border: 'border-blue-500/20', desc: 'Total Volume $20.000 - $80.000', icon: Star, benefits: ['Weekly Cashback'] },
    { name: 'Enforcer', color: 'text-red-500', bg: 'bg-red-500/10', border: 'border-red-500/20', desc: 'Total Volume $80.000 - $150.000', icon: Star, benefits: ['Daily Cashback', 'Weekly Cashback', 'Monthly Cashback'] },
    { name: 'Architect', color: 'text-yellow-500', bg: 'bg-yellow-500/10', border: 'border-yellow-500/20', desc: 'Total Volume $150.000 - $300.000', icon: Star, benefits: ['Daily Cashback', 'Weekly Cashback', 'Monthly Cashback'] },
    { name: 'Syndicate', color: 'text-purple-500', bg: 'bg-purple-500/10', border: 'border-purple-500/20', desc: 'Total Volume $300k - $1M+', icon: Star, benefits: ['Daily Cashback', 'Weekly Cashback', 'Monthly Cashback', 'VIP Group', 'Great chance to get a Rolex watch', 'Trip to Vegas and Dubai'] },
  ];
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
      ...
      >
        <div className="p-6 border-b border-white/10 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#a252f0]/20 flex items-center justify-center">
              <Star className="w-5 h-5 text-[#a252f0]" fill="currentColor" />
            </div>
            <div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center bg-black/20 ${tier.color}`}>
                      <tier.icon className="w-4 h-4" fill="currentColor" />
                    </div>
                    <div>
...
function ReferralModal({ isOpen, onClose, user }: { isOpen: boolean; onClose: () => void; user: string | null }) {
  ...
  const [fetchedReferralCode, setFetchedReferralCode] = useState<string>('');
  const [isLoadingCode, setIsLoadingCode] = useState(false);

  useEffect(() => {
    if (isOpen && user) {
      setIsLoadingCode(true);
      fetch('https://streetcoinz-login.streetcoinz.workers.dev/', {
        method: 'POST',
      ...
      .then(data => {
        if (data.referral_code) {
          setFetchedReferralCode(data.referral_code);
        }
      })
  ...
  const referralCode = fetchedReferralCode || (user ? btoa(user).substring(0, 8).toUpperCase() : 'STREET24');
          ...
          {activeTab === 'code' && (
            ...
            >
              ...
              <div className="bg-black/20 p-4 rounded-xl border border-white/5 mb-6">
                ...
                <div className="flex items-center gap-2 mb-4">
                  <code className="flex-1 text-lg font-bold text-center text-[#a252f0] bg-[#a252f0]/10 px-3 py-2.5 rounded-lg border border-[#a252f0]/20">
                    {isLoadingCode ? 'Loading...' : referralCode}
                  </code>
                  <button 
                    onClick={() => {
                ...
                <div className="flex items-center gap-2">
                  <code className="flex-1 text-sm text-white truncate bg-[#2a2a2a] px-3 py-2.5 rounded-lg border border-white/5">
                    {isLoadingCode ? 'Loading...' : referralLink}
                  </code>
                  <button 
                    onClick={handleCopy}
                    className="bg-[#a252f0] text-white p-2.5 rounded-lg hover:bg-[#8e44d6] transition-colors flex items-center justify-center shrink-0"
              ...
              <div className="grid grid-cols-5 gap-3">
                <button onClick={() => handleShare('twitter')} className="bg-white/5 text-gray-400 p-3 rounded-xl flex items-center justify-center hover:bg-[#1DA1F2]/20 hover:text-[#1DA1F2] transition-colors">
                ...
                <button onClick={() => handleShare('telegram')} className="bg-white/5 text-gray-400 p-3 rounded-xl flex items-center justify-center hover:bg-[#0088cc]/20 hover:text-[#0088cc] transition-colors">
                ...
                <button onClick={() => handleShare('whatsapp')} className="bg-white/5 text-gray-400 p-3 rounded-xl flex items-center justify-center hover:bg-[#25D366]/20 hover:text-[#25D366] transition-colors">
                ...
                <button onClick={() => handleShare('facebook')} className="bg-white/5 text-gray-400 p-3 rounded-xl flex items-center justify-center hover:bg-[#1877F2]/20 hover:text-[#1877F2] transition-colors">
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                </button>
                <button onClick={() => handleShare('native')} className="bg-white/5 text-gray-400 p-3 rounded-xl flex items-center justify-center hover:bg-white/20 hover:text-white transition-colors">
...
export default function App() {
  ...
  const [blockchainAssets, setBlockchainAssets] = useState<{ eth: number, bnb: number, matic: number, usdt: number, ethUsdt: number, bnbUsdt: number, maticUsdt: number, ethUsd: number, bnbUsd: number, maticUsd: number, usdtUsd: number, totalUsd: number }>({ eth: 0, bnb: 0, matic: 0, usdt: 0, ethUsdt: 0, bnbUsdt: 0, maticUsdt: 0, ethUsd: 0, bnbUsd: 0, maticUsd: 0, usdtUsd: 0, totalUsd: 0 });
  ...
  const [liveBets, setLiveBets] = useState<LiveBet[]>([
    ...
    { id: 6, game: 'Roulette', user: '0x8A1...B4C2', username: 'LuckyStrike', amount: 0.5, isWin: true, coin: 'SOL' },
    { id: 7, game: 'Mines', user: '0x2F9...D1E8', username: 'Anonim', amount: 100.00, isWin: false, coin: 'USDT' },
    { id: 8, game: 'Chicken', user: '0x4E3...A9B7', username: 'DegenTrader', amount: 2.5, isWin: true, coin: 'BNB' },
    { id: 9, game: 'SOL IN 5M', user: '0x6C5...F2A1', username: 'Anonim', amount: 10.0, isWin: false, coin: 'SOL' },
    { id: 10, game: 'Crash', user: '0x9B2...E4D6', username: 'Whale99', amount: 5.0, isWin: true, coin: 'ETH' },
  ]);
